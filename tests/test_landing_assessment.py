...
def test_no_warnings_or_blockers(client, db, phabdouble, auth0_mock):
    ...
    response = client.post(
        "/landings/dryrun",
        json=dict(revision_id="D{}".format(revision["id"]), diff_id=diff["id"]),
        headers=auth0_mock.mock_headers,
    )

    assert 200 == response.status_code
    assert "application/json" == response.content_type
    expected_json = {"confirmation_token": None, "warnings": [], "blockers": []}
...
def test_construct_assessment_dict_no_warnings_or_blockers():
    ...
    expected_dict = {"confirmation_token": None, "warnings": [], "blockers": []}
...
def test_token_with_warnings_is_not_none():
    assert LandingAssessment.hash_warning_list([{"id": "W0", "message": "oops"}])


def test_hash_with_different_list_order_is_equal():
    a = LandingAssessment.hash_warning_list(
        [
            {"id": "W1", "message": "oops 1"},
            {"id": "W2", "message": "oops 2"},
            {"id": "W3", "message": "oops 3"},
        ]
    )
...
def test_hash_with_same_id_different_warning_details_are_different():
    a = LandingAssessment.hash_warning_list(
        [{"id": "W0", "message": "revision 5 problem"}]
    )
    b = LandingAssessment.hash_warning_list(
        [{"id": "W0", "message": "revision 8 problem"}]
    )
    assert a != b
...
def test_hash_with_duplicate_ids_are_not_stripped():
    a = LandingAssessment.hash_warning_list(
        [{"id": "W0", "message": "same"}, {"id": "W0", "message": "same"}]
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            CANNED_USERINFO["EXPIRED_L3"],
            200,
            [{"id": "E002", "message": "Your Level 3 Commit Access has expired."}],
        ),
        (
            ...
            [
                {
                    "id": "E001",
                    "message": "You do not have a Mozilla verified email address.",
                }
            ],
        ),
    ],
...
def test_inactive_diff_blocks(db, client, phabdouble, transfactory, auth0_mock):
    d1 = phabdouble.diff()
...
def test_previously_landed_warns(db, client, phabdouble, transfactory, auth0_mock):
    diff = phabdouble.diff()
    revision = phabdouble.revision(diff=diff, repo=phabdouble.repo())
    db.session.add(
        Transplant(
...
def test_previously_landed_but_landed_since_still_warns(
    ...
    response = client.post(
        "/landings/dryrun",
        json=dict(revision_id="D{}".format(revision["id"]), diff_id=diff2["id"]),
        headers=auth0_mock.mock_headers,
        content_type="application/json",
    )
    assert response.status_code == 200
...
def test_author_planned_changes(client, db, phabdouble, auth0_mock):
    diff = phabdouble.diff()
    revision = phabdouble.revision(
        diff=diff, repo=phabdouble.repo(), status=RevisionStatus.CHANGES_PLANNED
    )
...
def test_unaccepted_warns(client, db, phabdouble, auth0_mock):
    diff = phabdouble.diff()
    revision = phabdouble.revision(
        diff=diff, repo=phabdouble.repo(), status=RevisionStatus.NEEDS_REVIEW
    )
...
def test_accepted_older_warns(client, db, phabdouble, auth0_mock):
    revision = phabdouble.revision(
        diff=phabdouble.diff(), repo=phabdouble.repo(), status=RevisionStatus.ACCEPTED
    )
    phabdouble.reviewer(revision, phabdouble.user(username="reviewer"))
    phabdouble.reviewer(revision, phabdouble.project("reviewer2"))

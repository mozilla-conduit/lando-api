...
def test_integrated_stack_endpoint_simple(client, phabdouble, mocked_repo_config, db):
    repo = phabdouble.repo()
...
def test_integrated_stack_endpoint_repos(client, phabdouble, mocked_repo_config, db):
    repo = phabdouble.repo()
    unsupported_repo = phabdouble.repo(name="not-mozilla-central")
    r1 = phabdouble.revision(repo=repo)
    r2 = phabdouble.revision(repo=repo, depends_on=[r1])

...
def test_get_revision_with_foreign_diff(client, phabdouble):
    ...
    response = client.get("/revisions/D{}?diff_id={}".format(r2["id"], d1["id"]))
...
def test_get_revision_multiple_reviewers(client, phabdouble):
    ...
    phabdouble.reviewer(revision, u2, status=ReviewerStatus.REJECTED, isBlocking=False)
    phabdouble.reviewer(revision, p1, status=ReviewerStatus.BLOCKING, isBlocking=True)

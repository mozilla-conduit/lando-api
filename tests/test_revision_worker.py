...
from landoapi.workers.revision_worker import (
    Processor,
    Supervisor,
    get_active_repos,
    get_stacks,
...
def test_get_stacks(phabdouble):
    ...
    assert sorted(test[1].nodes) == sorted([r1["phid"], r2["phid"], r3["phid"]])

    assert len(test[0].edges) == 0
    assert sorted(test[1].edges) == sorted(
...
def test_workers_integration(
    ...
    assert {r.status for r in revisions} == {RevisionStatus.WAITING}
    ...
    assert revision_1.predecessor is None
    ...
    revisions = Revision.query.all()
    assert len(revisions) == 3
    assert {r.status for r in revisions} == {RevisionStatus.READY}
...
def test_workers_integration_modify_stacks_simple(
    ...
    assert revision_4.predecessor is None
